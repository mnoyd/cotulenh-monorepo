...
- [ ] Task 1: Database migration — friendships table (AC: all)
  - [ ] 1.1 Create `supabase/migrations/002_friendships.sql`
  - [ ] 1.2 Define table: `id` (uuid PK), `user_a` (uuid FK → profiles), `user_b` (uuid FK → profiles), `status` (text: pending/accepted/blocked), `initiated_by` (uuid FK → profiles), `created_at` (timestamptz), `updated_at` (timestamptz)
  - [ ] 1.3 Constraint: `user_a < user_b` to enforce canonical ordering (prevent duplicate rows for same pair)
  - [ ] 1.4 Unique constraint on `(user_a, user_b)`
  - [ ] 1.5 Enable RLS: users can only read/write rows where they are `user_a` or `user_b`
  - [ ] 1.6 Create indexes on `user_a`, `user_b`, and `status`

- [ ] Task 2: Friends lib module (AC: 2, 3, 4, 5)
  - [ ] 2.1 Create `$lib/friends/types.ts` — FriendshipRow, FriendshipStatus, FriendSearchResult types
  - [ ] 2.2 Create `$lib/friends/queries.ts` — searchUsers(query, currentUserId), sendFriendRequest(fromUserId, toUserId), getFriendshipStatus(userA, userB)
  - [ ] 2.3 searchUsers: query `profiles.display_name` with `ilike` filter, exclude self, join with friendships to determine relationship status per result
  - [ ] 2.4 sendFriendRequest: insert into friendships with canonical `user_a < user_b` ordering, set `initiated_by`
  - [ ] 2.5 Transform `snake_case` DB columns to `camelCase` at query boundary

- [ ] Task 3: Friends page route and server load (AC: 1, 6, 7)
  - [ ] 3.1 Create `/user/friends/+page.server.ts` with load function
  - [ ] 3.2 Load current user's friends list (friendships where status = 'accepted') with display names from profiles join
  - [ ] 3.3 Create search action: accept query string, return matching profiles with friendship status
  - [ ] 3.4 Create sendRequest action: validate, insert friendship, return success/error

- [ ] Task 4: Friends page UI (AC: 1, 2, 3, 4, 5, 6, 7, 8, 9)
  - [ ] 4.1 Create `/user/friends/+page.svelte`
  - [ ] 4.2 Search input with debounced (300ms) client-side fetch to search action
  - [ ] 4.3 Search results dropdown showing display name + relationship button (Send Request / Pending / Friends)
  - [ ] 4.4 Friends list section showing accepted friends with display name
  - [ ] 4.5 Empty state: "No friends yet" + "Search for players" with inline search
  - [ ] 4.6 Optimistic update: button changes immediately on send, reverts on error
  - [ ] 4.7 Responsive layout: mobile full-width cards, tablet two-column, desktop three-column

- [ ] Task 5: Shared components (AC: 9)
  - [ ] 5.1 Create `$lib/components/PlayerCard.svelte` — avatar (initial letter), display name, action button slot
  - [ ] 5.2 Create `$lib/components/OnlineIndicator.svelte` — green dot / no dot (prepared for Story 3.3, hidden for now)

- [ ] Task 6: i18n translations (AC: 8)
  - [ ] 6.1 Add friends translation keys to `$lib/i18n/types.ts`
  - [ ] 6.2 Add English translations to `$lib/i18n/locales/en.ts`
  - [ ] 6.3 Add Vietnamese translations to `$lib/i18n/locales/vi.ts`

- [ ] Task 7: Tests (AC: all)
  - [ ] 7.1 Migration test: verify table, constraints, RLS policies
  - [ ] 7.2 Query tests: searchUsers returns correct results with relationship status, sendFriendRequest creates row with canonical ordering
  - [ ] 7.3 Server action tests: search action, sendRequest action (success, duplicate, self-request)
  - [ ] 7.4 Validation edge cases: minimum 2 chars for search, self-exclusion

- [ ] Task 8: Navigation integration (AC: 1)
  - [ ] 8.1 Add "Friends" link to sidebar navigation in `+layout.svelte` pointing to `/user/friends`
  - [ ] 8.2 Add i18n keys for nav.friends
