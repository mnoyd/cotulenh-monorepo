...
// Import CoTuLenh for semantic validation
// Note: This is placed after other imports to avoid circular dependency issues
// The validateFenSemantics function uses this sparingly to avoid recursion
...
// ============================================================================
// FEN VALIDATION PUBLIC API
// ============================================================================

/**
 * Validates a FEN string comprehensively.
 *
 * Performs structural validation (token count, rank count, file count,
 * parentheses balance) and optionally semantic validation (commander count,
 * terrain validity, combination validity) and round-trip testing.
 *
 * @param fen - The FEN string to validate
 * @param options - Validation options
 * @returns ValidationResult with all errors found
 * @throws CotulenhError if throwOnError is true and validation fails
 *
 * @example
 * ```ts
 * // Basic validation - collect all errors
 * const result = validateFen('6c4/... r - - 0 1')
 * if (!result.valid) {
 *   console.log('Errors:', result.errors)
 * }
 *
 * // With round-trip test
 * const result2 = validateFen(fen, { roundTripTest: true })
 *
 * // Throw on first error (backward compatible)
 * try {
...
export function validateFen(
  fen: string,
  options: ValidateFenOptions = {},
): ValidationResult {
  const opts: Required<ValidateFenOptions> = {
  ...
  // Check if we're in a semantic validation call to prevent infinite recursion
  // @ts-ignore - accessing private flag
  const _doingSemanticValidation =
    (validateFenSemantics as any)._doingSemanticValidation || false

  // Phase 1: Structural validation (no game state, fast)
  errors.push(...validateFenStructure(fen))

  // Early exit if structural errors found - semantic checks will be unreliable
  const hasStructuralErrors = errors.some(
    (e) => e.category === 'structure' || e.category === 'lexical',
  )

  // Only do semantic validation if not already in a semantic validation call
  if (
    !hasStructuralErrors &&
    opts.checkSemantics &&
    !_doingSemanticValidation
  ) {
    // Phase 2: Semantic validation (uses CoTuLenh instance)
    errors.push(...validateFenSemantics(fen))
  }

  if (opts.roundTripTest && !errors.some((e) => e.category === 'structure')) {
    // Phase 3: Round-trip validation
    errors.push(...validateFenRoundTrip(fen))
  }
  ...
  if (opts.throwOnError && !result.valid) {
    // Throw the first error with full context
    const firstError = errors[0]
    throw createError(firstError.code, firstError.message, firstError.location)
  }
...
/**
 * Validates a complete FEN string (legacy API).
 *
 * @param fen - The FEN string to validate
 * @throws CotulenhError if the FEN is invalid
 * @deprecated Use validateFen() with throwOnError: true for better error reporting
 */
export function validateFenLegacy(fen: string): void {
  const result = validateFen(fen, { throwOnError: true })
  if (!result.valid) {
    // This should never be reached due to throwOnError, but for type safety:
    throw createError(
      result.errors[0]?.code || ErrorCode.FEN_INVALID_FORMAT,
      result.errors[0]?.message || 'Invalid FEN',
...
// ============================================================================
// STRUCTURAL VALIDATION (Fast, no side effects)
// ============================================================================

/**
 * Validates FEN structure without creating game state.
 * Checks token count, rank count, file count, parentheses, and valid characters.
 *
 * @param fen - The FEN string to validate
 * @returns Array of validation errors
 */
function validateFenStructure(fen: string): ValidationError[] {
  const errors: ValidationError[] = []
  ...
  if (tokens.length < 6) {
    ...
    return errors // Can't proceed without proper tokens
  }
  ...
  if (turn !== 'r' && turn !== 'b') {
    errors.push({
      ...
      category: 'structure',
      location: { tokenIndex: 1, context: turn },
    })
  }

  // Validate castling field (always '-' for this variant)
  if (castling !== '-') {
    errors.push({
      code: ErrorCode.FEN_INVALID_FORMAT,
      message: `Castling field must be '-', got '${castling}'`,
      category: 'structure',
      location: { tokenIndex: 2, context: castling },
    })
  }

  // Validate en passant field (always '-' for this variant)
  if (enPassant !== '-') {
    errors.push({
      code: ErrorCode.FEN_INVALID_FORMAT,
      message: `En passant field must be '-', got '${enPassant}'`,
      category: 'structure',
      location: { tokenIndex: 3, context: enPassant },
    })
  }

  // Validate half moves is a non-negative integer
  const halfMovesNum = parseInt(halfMoves, 10)
  if (isNaN(halfMovesNum) || halfMovesNum < 0) {
    errors.push({
      code: ErrorCode.FEN_INVALID_FORMAT,
      message: `Invalid half move count '${halfMoves}', expected non-negative integer`,
      category: 'structure',
      location: { tokenIndex: 4, context: halfMoves },
    })
  }

  // Validate move number is a positive integer
  const moveNumberNum = parseInt(moveNumber, 10)
  if (isNaN(moveNumberNum) || moveNumberNum < 1) {
    errors.push({
      code: ErrorCode.FEN_INVALID_FORMAT,
      message: `Invalid move number '${moveNumber}', expected positive integer`,
      category: 'structure',
      location: { tokenIndex: 5, context: moveNumber },
    })
  }

  // Validate piece placement
  errors.push(...validatePiecePlacement(placement))

  return errors
}

/**
 * Validates the piece placement field specifically.
 *
 * @param placement - The piece placement part of FEN (before first space)
 * @returns Array of validation errors
 */
function validatePiecePlacement(placement: string): ValidationError[] {
  const errors: ValidationError[] = []
  ...
  for (let r = 0; r < 12; r++) {
    const rankErrors = validateRank(ranks[r], 12 - r)
 ...
 *
 * @param rank - The rank string (e.g., "6c4", "2(FTM)3")
 * @param displayRank - The display rank number (1-12)
 * @returns Array of validation errors
 */
function validateRank(rank: string, displayRank: number): ValidationError[] {
  const errors: ValidationError[] = []
  let col = 0
  let inParens = false
  let parenDepth = 0
  ...
  for (let i = 0; i < rank.length; i++) {
    ...
    if (isDigit(char)) {
      // Parse multi-digit numbers (10, 11)
      let numStr = char
      if (char === '1' && i + 1 < rank.length && isDigit(rank[i + 1])) {
        numStr += rank[++i]
      }
      ...
      col++ // Closing paren advances position
      lastWasPlus = false
    } else if (isValidPieceSymbol(char.toLowerCase())) {
      if (!inParens) {
        col++
      }
      lastWasPlus = false
  ...
  // Check for dangling heroic marker
  if (lastWasPlus) {
    errors.push({
      code: ErrorCode.FEN_INVALID_FORMAT,
...
/**
 * Checks if a character is a valid piece symbol.
 *
 * @param char - The character to check (lowercase)
 * @returns True if the character is a valid piece symbol
 */
function isValidPieceSymbol(char: string): boolean {
  return VALID_PIECE_TYPES[char] === true
}

// ============================================================================
// SEMANTIC VALIDATION (Uses CoTuLenh instance)
// ============================================================================

/**
 * Validates FEN semantics by attempting to load the position.
 * Catches terrain, combination, and commander limit errors.
 *
 * @param fen - The FEN string to validate
 * @returns Array of validation errors
 */
function validateFenSemantics(fen: string): ValidationError[] {
  const errors: ValidationError[] = []

  try {
    // Load the FEN - put() will validate terrain and combinations
    // Use a flag to prevent infinite recursion
    // @ts-ignore - accessing private method
    const _doingSemanticValidation =
      (validateFenSemantics as any)._doingSemanticValidation || false
    if (_doingSemanticValidation) {
      // Skip semantic validation if we're already in a semantic validation call
      return errors
    }
    // @ts-ignore - setting private flag
    ;(validateFenSemantics as any)._doingSemanticValidation = true
    ...
    try {
      ...
      if (commanders.r !== 1) {
        errors.push({
          code: ErrorCode.FEN_INVALID_FORMAT,
          message: `Expected 1 red commander, found ${commanders.r}`,
          category: 'semantic',
        })
      }
      ...
      if (commanders.b !== 1) {
        errors.push({
          code: ErrorCode.FEN_INVALID_FORMAT,
          message: `Expected 1 blue commander, found ${commanders.b}`,
          category: 'semantic',
        })
      }
    } finally {
      // @ts-ignore - clearing private flag
      ;(validateFenSemantics as any)._doingSemanticValidation = false
    }
  } catch (e) {
    // Convert thrown errors to ValidationError
...
/**
 * Categorizes an error code into a ValidationErrorCategory.
 *
...
function categorizeErrorCode(code: ErrorCode): ValidationErrorCategory {
  if (code === ErrorCode.BOARD_INVALID_TERRAIN) return 'terrain'
  if (code === ErrorCode.COMBINATION_FAILED) return 'combination'
  if (code === ErrorCode.COMMANDER_LIMIT_EXCEEDED) return 'semantic'
  if (code === ErrorCode.FEN_INVALID_RANK_COUNT) return 'structure'
  if (code === ErrorCode.FEN_INVALID_FILE_COUNT) return 'structure'
  if (code === ErrorCode.FEN_MISMATCH_PARENTHESES) return 'structure'
  if (code === ErrorCode.FEN_INVALID_PIECE) return 'lexical'
  return 'semantic'
}
...
/**
 * Validates FEN by round-trip test (parse → FEN → compare).
 *
 * @param fen - The FEN string to validate
 * @returns Array of validation errors
 */
function validateFenRoundTrip(fen: string): ValidationError[] {
  const errors: ValidationError[] = []

  let CoTuLenh: any
  try {
    const module = require('./cotulenh.js')
    CoTuLenh = module.CoTuLenh
  } catch (e) {
    return errors
  }
  ...
  try {
    ...
    const reconstructed = game.fen()

    const originalPlacement = fen.split(/\s+/)[0]
    const reconstructedPlacement = reconstructed.split(/\s+/)[0]

    if (originalPlacement !== reconstructedPlacement) {
      errors.push({
        code: ErrorCode.FEN_INVALID_FORMAT,
        message: 'Round-trip failed: reconstructed FEN differs from input',
        category: 'roundtrip',
        location: {
          context: `Original: ${originalPlacement}, Reconstructed: ${reconstructedPlacement}`,
        },
      })
    }
  } catch {
    // If we can't load, semantic validation already caught this
  }
