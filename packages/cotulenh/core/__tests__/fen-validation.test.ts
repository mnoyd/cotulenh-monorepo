...
const checkFen = CoTuLenh.checkFen.bind(CoTuLenh)
...
describe('FEN Validation', () => {
  describe('structural validation', () => {
    it('accepts a valid FEN string', () => {
      const result = checkFen(INITIAL_FEN)
    ...
    it('accepts FEN with all empty squares (11)', () => {
      ...
      const result = checkFen(fen, { checkSemantics: false })
      expect(result.valid).toBe(true)
    })
  ...
  describe('semantic validation', () => {
    it('accepts valid FEN with semantic checks enabled', () => {
      const result = checkFen(INITIAL_FEN, { checkSemantics: true })
      expect(result.valid).toBe(true)
    })
    ...
    it('detects missing commanders with semantic checks', () => {
      const fen = '11/11/11/11/11/11/11/11/11/11/11/11 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(false)
      expect(
        result.errors.some((e) =>
    ...
    it('rejects FEN with two commanders of same color', () => {
      // Two red commanders (C) at different positions - files g and h (land terrain)
      const fen = '6C4/11/11/11/11/11/11/11/11/11/11/6CC3 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(false)
      expect(
        result.errors.some((e) => e.code === 'COMMANDER_LIMIT_EXCEEDED'),
      ).toBe(true)
    })

    it('rejects FEN with navy on land terrain', () => {
      // Navy at file g (pure land) - Navy can only be on files a, b, c or river squares
      // Valid positions: c3 has both commanders, n at g3 is invalid
      const fen = '6c4/11/11/11/11/11/11/11/11/6n4/11/6C4 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(false)
      expect(
        result.errors.some((e) => e.code === 'BOARD_INVALID_TERRAIN'),
      ).toBe(true)
    })

    it('rejects FEN with land unit on water terrain', () => {
      // Infantry at file a (pure navy zone) - land units cannot be on files a, b
      const fen = 'I10/11/11/11/11/11/11/11/11/11/11/6Cc3 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(false)
      expect(
        result.errors.some((e) => e.code === 'BOARD_INVALID_TERRAIN'),
    ...
    it('rejects FEN with invalid stack combination', () => {
      // Invalid: Navy cannot carry Artillery (only AIR_FORCE in slot1, COMMANDER/INFANTRY/MILITIA/TANK in slot2)
      // Navy must be on water terrain (file a, b, or c)
      const fen = '6c4/11/11/11/11/11/11/11/11/(na)10/11/6C4 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.code === 'COMBINATION_FAILED')).toBe(
        true,
      )
    })

    it('accepts FEN with valid navy stack', () => {
      // Valid: Navy carrying Air Force (slot1) and Tank (slot2) on file a (navy terrain)
      const fen = '6c4/11/11/11/11/11/11/11/11/(nft)10/11/6C4 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(true)
    })

    it('accepts FEN with valid tank stack', () => {
      // Valid: Tank carrying Infantry on land terrain (file g)
      const fen = '6c4/11/11/11/11/11/11/11/11/11/11/6C(TI)3 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(true)
    })

    it('accepts FEN with valid engineer stack', () => {
      // Valid: Engineer carrying Artillery on land terrain (file g)
      const fen = '6c4/11/11/11/11/11/11/11/11/11/11/6C(EA)3 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(true)
    })

    it('accepts FEN with valid air force stack', () => {
      // Valid: Air Force carrying Tank and Infantry - air force can be anywhere
      const fen = '6c4/11/11/11/11/11/11/11/11/11/11/6C(FTI)3 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(true)
    })

    it('accepts FEN with navy on mixed terrain (file c)', () => {
      // Navy at c3 (mixed terrain - valid for navy)
      const fen = '6c4/11/11/11/11/11/11/11/11/2n8/11/6C4 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(true)
    })

    it('accepts FEN with land unit on mixed terrain (file c)', () => {
      // Infantry at c3 (mixed terrain - valid for land units)
      const fen = '6c4/11/11/11/11/11/11/11/11/2i8/11/6C4 r - - 0 1'
      const result = checkFen(fen, { checkSemantics: true })
      expect(result.valid).toBe(true)
    })
  })
  ...
  describe('round-trip validation', () => {
    it('passes round-trip test for valid FEN', () => {
      const result = checkFen(INITIAL_FEN, { roundTripTest: true })
      expect(result.valid).toBe(true)
    })

    it('does not run round-trip test when structural errors exist', () => {
      const fen = '11/11/11/11/11/11/11/11/11/11/11 r - - 0 1'
      const result = checkFen(fen, { roundTripTest: true })
      expect(result.valid).toBe(false)
  ...
  describe('throwOnError option', () => {
    it('does not throw when throwOnError is false (default)', () => {
      const fen = '11/11/11/11/11/11/11/11/11/11/11/11 w - - 0 1'
      expect(() => checkFen(fen)).not.toThrow()
    })

    it('throws when throwOnError is true and FEN is invalid', () => {
      const fen = '11/11/11/11/11/11/11/11/11/11/11/11 w - - 0 1'
      expect(() => checkFen(fen, { throwOnError: true })).toThrow()
    })

    it('does not throw when throwOnError is true and FEN is valid', () => {
      expect(() => checkFen(INITIAL_FEN, { throwOnError: true })).not.toThrow()
    })
  })
