import { cancelMove, getKeyAtDomPos, getSnappedKeyAtDomPos, redPov, unselect } from './board.js';
...
export function processDraw(state: State): void {
  requestAnimationFrame(() => {
    ...
    if (cur) {
      const keyAtDomPos = getKeyAtDomPos(cur.pos, redPov(state), state.dom.bounds());
      if (!keyAtDomPos) {
        cur.snapToValidMove = false;
      }
      const mouseSq = cur.snapToValidMove
        ? getSnappedKeyAtDomPos(cur.pos, redPov(state), state.dom.bounds())
        : keyAtDomPos;
