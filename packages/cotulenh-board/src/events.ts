...
export function bindBoard(s: State, onResize: () => void): void {
  ...
  if ('ResizeObserver' in globalThis) new ResizeObserver(onResize).observe(s.dom.elements.wrap);
...
export function bindDocument(s: State, onResize: () => void): cg.Unbind {
  ...
  if (!s.viewOnly) {
    ...
    unbinds.push(
      ...['touchmove', 'mousemove'].map(ev => unbindable(document, ev, onmove as EventListener)),
      ...['touchend', 'mouseup'].map(ev => unbindable(document, ev, onend as EventListener)),
